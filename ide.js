...
downloadZipBtn.addEventListener('click', () => {
    ...
    const dataStr = 'text/json;charset=utf-8,' + encodeURIComponent(JSON.stringify(projectFiles, null, 2));
...
function exportBots() {
    const dataStr = 'text/json;charset=utf-8,' + encodeURIComponent(JSON.stringify(customBots, null, 2));
    const dlAnchor = document.createElement('a');
    dlAnchor.setAttribute('href', dataStr);
...
function downloadCommitJson(commit) {
    const dataStr = 'text/json;charset=utf-8,' + encodeURIComponent(JSON.stringify(commit, null, 2));
...
// Initialize workspace with Git history - loads ALL data from VFS
async function initWorkspace() {
    console.log('[IDE START] Initializing workspace from VFS...');

    // Wait for VFS to be ready
    await window.projectDB.ensureReady();

    // Try to load files from IndexedDB first
    let filesFromVFS = null;
    try {
        filesFromVFS = await window.projectDB.getAllFiles();
        console.log(`[IDE START] Loaded ${filesFromVFS.length} files from VFS`);
    } catch (e) {
        console.error('[IDE START] Failed to load from VFS:', e);
    }

    if (filesFromVFS && filesFromVFS.length > 0) {
        // Convert array to object
        projectFiles = {};
        filesFromVFS.forEach(f => {
            projectFiles[f.path] = f.content;
        });
    } else {
        // Fallback to localStorage
        console.log('[IDE START] No VFS data, trying localStorage...');
        const saved = localStorage.getItem('ide_project_files');
        if (saved) {
            try {
                projectFiles = JSON.parse(saved);
                console.log('[IDE START] Loaded from localStorage');
            } catch (e) {
                projectFiles = { ...DEFAULT_FILES };
            }
        } else {
    ...
    // Initialize Monaco Editor FIRST, then load content from VFS
    initMonacoEditor();

    // Load active file content from VFS into Monaco
    await loadActiveFileFromVFS();

    checkApiConfiguration();
    console.log('[IDE START] Workspace initialization complete');
}

// Load active file content from VFS into Monaco Editor
async function loadActiveFileFromVFS() {
    try {
        const content = await window.projectDB.getFile(activeFileName);
        if (content !== null) {
            console.log(`[MONACO REACTION] Loading ${activeFileName} from VFS (${content.length} bytes)`);
            projectFiles[activeFileName] = content;
            if (codeEditor) {
                const model = codeEditor.getModel();
                if (model) {
                    monaco.editor.setModelLanguage(model, getLanguageForFile(activeFileName));
                    codeEditor.executeEdits('vfs-load', [{
                        range: model.getFullModelRange(),
                        text: content
                    }]);
                    console.log('[MONACO REACTION] Content loaded via executeEdits');
                }
            }
        } else {
            // File not in VFS, use projectFiles
            if (codeEditor && projectFiles[activeFileName]) {
                const model = codeEditor.getModel();
                if (model) {
                    codeEditor.executeEdits('vfs-load', [{
                        range: model.getFullModelRange(),
                        text: projectFiles[activeFileName]
                    }]);
                }
            }
        }
    } catch (e) {
        console.error('[VFS ERROR] Failed to load active file:', e);
...
async function saveWorkspace() {
    ...
    // Also save all files to IndexedDB VFS
    try {
        const savePromises = Object.entries(projectFiles).map(([path, content]) => {
            return window.projectDB.saveFile(path, content);
        });
        await Promise.all(savePromises);
        console.log('[VFS WRITE] Workspace saved to IndexedDB');
    } catch (e) {
        console.error('[VFS ERROR] Failed to save to IndexedDB:', e);
    }
}
...
async function selectFile(filename) {
    ...
    try {
        const content = await window.projectDB.getFile(filename);
        const fileContent = content !== null ? content : (projectFiles[filename] || "");

        if (codeEditor) {
            console.log(`[MONACO REACTION] Switching to ${filename} (${fileContent.length} bytes)`);
            const model = codeEditor.getModel();
            if (model) {
                monaco.editor.setModelLanguage(model, getLanguageForFile(filename));
                const currentValue = codeEditor.getValue();
                if (currentValue !== fileContent) {
                    ...
                    console.log('[MONACO REACTION] File switched via executeEdits');
                }
            } else {
                codeEditor.setValue(fileContent);
            }
        ...
        console.error('[VFS ERROR] Failed to load file on switch:', e);
        // Fallback
        if (codeEditor) {
            codeEditor.setValue(projectFiles[filename] || "");
        }
    }
}
...
function initializeMonaco() {
    ...
    require(['vs/editor/editor.main'], function () {
        ...
        // Debounced save to VFS on content change
        ...
        codeEditor.onDidChangeModelContent(() => {
            ...
            if (activeFileName && projectFiles[activeFileName] !== undefined) {
                ...
                // Debounced write to VFS
                clearTimeout(saveTimeout);
                saveTimeout = setTimeout(async () => {
                    try {
                        await window.projectDB.saveFile(activeFileName, projectFiles[activeFileName]);
                        console.log(`[VFS WRITE] ${activeFileName} saved from Monaco change`);
                    } catch (e) {
                        console.error('[VFS ERROR] Failed to save on change:', e);
                    }
                }, 500);
            }
        });
        ...
        // Note: selectFile is called separately after initMonacoEditor in initWorkspace
    });
}
...
async function getPreviewHtml() {
    // Read files from VFS for preview (not from memory)
    let html = projectFiles["index.html"] || "<h1>No index.html found</h1>";

    console.log('[VFS READ] Building preview from projectFiles');
...
async function runPreview() {
    const content = await getPreviewHtml();
    projectIframe.srcdoc = content;
    setupErrorListener(); // Initialize error detection for this preview
    console.log('[PREVIEW] Updated with latest VFS content');
}
...
async function handleStreamingResponse(response, type) {
    ...
    while (!done) {
        ...
        if (value) {
            ...
            // ЯРКОЕ ЛОГИРОВАНИЕ STREAM CHUNK
            console.log(`%c[AI STREAM CHUNK] Received ${chunk.length} bytes`, 'color: #ff9900; font-weight: bold; background: #332200; padding: 2px 5px;');
...
Do not include any extra text outside of the file output blocks unless you are providing a short explanation. If you need to retry, respond with file outputs only.`;
        ...
        try {
            ...
            // ЯРКОЕ ЛОГИРОВАНИЕ AI PARSER RESULT
            console.log(`%c[AI PARSER RESULT] Parsing ${rawText.length} bytes of AI response`, 'color: #ffff00; font-weight: bold; background: #333300; padding: 2px 5px;');
            ...
            if (Object.keys(parsed.files).length > 0) {
                console.log(`%c[AI PARSER RESULT] Successfully extracted ${Object.keys(parsed.files).length} files: ${Object.keys(parsed.files).join(', ')}`, 'color: #00ff00; font-weight: bold; background: #003300; padding: 2px 5px;');
            } else {
...
function buildWorkflowFeedback(appliedFiles) {
    // Build feedback by reading current state from VFS (not memory)
    const fileNames = Object.keys(appliedFiles).join(', ');

    console.log(`[VFS READ] Building workflow feedback for files: ${fileNames}`);
...
async function applyProposedChanges() {
     ...
     console.log(`%c[AI PARSER RESULT] Applying ${Object.keys(proposedChanges).length} files from AI response`, 'color: #ffff00; font-weight: bold; background: #333300; padding: 2px 5px;');
     ...
     // 2. Persist workspace state to localStorage and IndexedDB VFS
     await saveWorkspace();

     // Also explicitly save each changed file to IndexedDB to trigger vfs:file-updated events
     for (const filepath of appliedFiles) {
         try {
             await window.projectDB.saveFile(filepath, projectFiles[filepath]);
             console.log(`%c[VFS WRITE] ${filepath} (${projectFiles[filepath].length} bytes) via applyProposedChanges`, 'color: #00ff00; font-weight: bold; background: #003300; padding: 2px 5px;');
         } catch (e) {
             console.error(`[VFS ERROR] Failed to save ${filepath} to IndexedDB:`, e);
         }
     }
     ...
     if (codeEditor && projectFiles[activeFileName] !== undefined && appliedFiles.includes(activeFileName)) {
         ...
         const model = codeEditor.getModel();
         if (model) {
             codeEditor.executeEdits('apply-changes', [{
                 range: model.getFullModelRange(),
                 text: projectFiles[activeFileName]
             }]);
             console.log('[MONACO REACTION] Updated via executeEdits in applyProposedChanges');
         } else {
             codeEditor.setValue(projectFiles[activeFileName]);
         }
     ...
     // Monaco updates are handled by vfs:file-updated event listener
     runPreview();
 }
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log('[IDE START] DOMContentLoaded - Starting initialization...');

    // Initialize IndexedDB VFS first
    await window.projectDB.ensureReady();
    console.log('[VFS SYSTEM] Ready for operations');

    // Initialize workspace (loads from VFS)
    await initWorkspace();

    // Listen for VFS file updates and update Monaco editor reactively
    window.addEventListener('vfs:file-updated', (event) => {
        const { path, content } = event.detail;

        console.log(`%c[VFS EVENT] Received vfs:file-updated for ${path}`, 'color: #ff00ff; font-weight: bold; background: #330033; padding: 2px 5px;');
        ...
        if (path === activeFileName && codeEditor) {
            ...
            if (model) {
                ...
                codeEditor.executeEdits('vfs-event-update', [{
                    range: model.getFullModelRange(),
                    text: content
                }]);
                ...
                console.log(`%c[MONACO REACTION] Updated via vfs:file-updated event (${content.length} bytes)`, 'color: #00ffff; font-weight: bold; background: #003333; padding: 2px 5px;');
            } else {
    ...
    window.addEventListener('vfs:file-deleted', (event) => {
        const { path } = event.detail;
        console.log(`[VFS EVENT] Received vfs:file-deleted for ${path}`);
    ...
    // Listen for workspace clear events
    window.addEventListener('vfs:cleared', () => {
        console.log('[VFS EVENT] Workspace cleared');
        projectFiles = { ...DEFAULT_FILES };
        renderFileList();
